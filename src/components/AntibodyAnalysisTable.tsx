...
import { analysisColumns, analysisCsv, analysisFasta, analysisIssues, analysisSequence, analysisValues, compareAnalysisValues, type AnalysisGroup } from "@/antibody-analysis"
...
export default function AntibodyAnalysisTable({ group, columns, origins, onInspect }: {
  ...
  function download(format: "csv" | "fasta") {
    if (url.current) URL.revokeObjectURL(url.current)
    url.current = URL.createObjectURL(new Blob([format === "csv" ? analysisCsv(group) : analysisFasta(group, selected)], { type: format === "csv" ? "text/csv;charset=utf-8" : "text/plain;charset=utf-8" }))
    const link = document.createElement("a")
    link.href = url.current
    link.download = `antibody-analysis-${group.id.replace(/[^a-zA-Z0-9_-]/g, "_")}.${format}`
    link.click()
  }
  return <section aria-label={`Analysis results: ${group.id}`} className="min-w-0 space-y-4 rounded-xl border p-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="text-xl font-semibold">{group.id}</h3><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => download("csv")}><Download aria-hidden="true" />Download CSV</Button><Button variant="outline" disabled={!selected.size} onClick={() => download("fasta")}><Download aria-hidden="true" />Download selected pairs</Button></div></div>
    <p className="text-sm text-muted-foreground" role="status">{selected.size} selected · FASTA preserves input order and includes selected standalone chains.{selected.size ? <> <button className="cursor-pointer underline underline-offset-4" onClick={() => setSelected(new Set())} type="button">Clear selection</button></> : null}</p>
    ...
    <div className="overflow-x-auto rounded-lg border" tabIndex={0} aria-label={`${group.id} table; scroll for more columns`}>
      <table className="w-full text-left text-sm">
        ...
        <thead className="bg-muted/40"><tr><th scope="col" className="border-b px-3 py-3"><span className="sr-only">Select</span></th>{columns.map((column) => <th key={column.name} scope="col" className="whitespace-nowrap border-b px-3 py-3" aria-sort={sort?.name === column.name ? sort.descending ? "descending" : "ascending" : "none"}>
        ...
        <tbody className="divide-y">{rows.slice(currentPage * 50, (currentPage + 1) * 50).map(({ entry, index, values }) => <tr key={index}>
          <td className="px-3 py-3 align-top"><input aria-label={`Select ${entry.id}`} type="checkbox" checked={selected.has(index)} disabled={!analysisSequence(entry)} title={!analysisSequence(entry) ? "No usable sequence to export" : undefined} onChange={(event) => setSelected((current) => { const next = new Set(current); if (event.target.checked) next.add(index); else next.delete(index); return next })} /></td>
          {columns.map((column) => <td key={column.name} className="px-3 py-3 align-top">
            {column.name === "issues" ? <div className="min-w-52 max-w-80 leading-6">{analysisIssues(entry).length ? <ul className="space-y-2">{analysisIssues(entry).map((issue, i) => <li key={i}>{issue}</li>)}</ul> : <span className="text-muted-foreground">None</span>}</div> : column.name === "id" ? <div className="min-w-36 max-w-64 space-y-2">
              <p className="break-words font-medium">{entry.id}</p>
              <div className="flex flex-wrap gap-2">{(["vh", "vl", "unassigned"] as const).map((role) => entry[role] ? <button key={role} title={`Inspect ${role === "unassigned" ? "unassigned chain" : role.toUpperCase()} sequence`} className="cursor-pointer rounded border px-2 py-1 text-xs hover:bg-accent" type="button" onClick={() => onInspect(entry[role]!.sequence, `${entry.id} · ${role === "unassigned" ? "Unassigned chain" : role.toUpperCase()}`)}>{role === "unassigned" ? "Unassigned chain" : role.toUpperCase()}</button> : null)}</div>
