...
export default function HumanizationResults({ jobId }: { jobId: string }) {
  ...
  const visibleColumns = data?.columns.filter((column) => !hiddenColumns.includes(column.name)) ?? []
  ...
  return <Card className="mt-6 min-w-0">
    <CardHeader>
      ...
      <p className="text-sm text-muted-foreground">Click a column header to sort; click again to reverse the order. Panel order remains the workflow’s per-parent ranking. Missing values sort last; ties use parent and candidate IDs. The full selection.csv is included in the result archive.</p>
    </CardHeader>
          ...
          <Menu.Portal>
            <Menu.Positioner align="start" className="z-50" sideOffset={8}>
              <Menu.Popup className="max-h-[min(24rem,var(--available-height))] w-80 max-w-[calc(100vw-2rem)] overflow-y-auto rounded-lg border bg-popover p-1 text-popover-foreground shadow-lg outline-none">
                <Menu.Item className="cursor-pointer rounded px-3 py-2 font-medium outline-none data-highlighted:bg-accent" closeOnClick={false} onClick={() => setHiddenColumns([])}>Show all columns</Menu.Item>
                <Menu.Separator className="my-1 border-t" />
                {data?.columns.map((column) => <Menu.CheckboxItem checked={!hiddenColumns.includes(column.name)} className="flex cursor-pointer items-start gap-2 rounded px-3 py-2 text-sm outline-none data-highlighted:bg-accent" closeOnClick={false} key={column.name} onCheckedChange={(checked) => setHiddenColumns((current) => checked ? current.filter((name) => name !== column.name) : [...current, column.name])}>
      ...
      {data ? <>
        <div aria-label="Candidate table; scroll horizontally for all columns" className="max-w-full overflow-x-auto rounded-lg border focus-visible:ring-2 focus-visible:ring-ring" tabIndex={0}>
          <table aria-busy={loadingPage} className="w-full text-left text-sm">
            ...
            <tbody aria-hidden={loadingPage || undefined} className={loadingPage ? "invisible divide-y" : "divide-y"}>{data.rows.map((row, index) => <tr key={`${row.parent_id}:${row.candidate_id}:${data.offset + index}`}>
