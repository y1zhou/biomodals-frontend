...
test("MVP password, jobs, download, cancellation, and sign-out", async ({
  ...
  await expect.poll(async () => (await browserStats()).preflight_versions.slice(0, 4)).toEqual([7, 1, 1, 1])
  ...
  // Cluster cumulative production through the real owner/CSRF/lifecycle routes.
  const clusteringSource = await (await context.request.get(`/api/v1/jobs/${chained}`)).json()
  ...
  await page.getByLabel("Job name (optional)", { exact: true }).fill("Browser clusters")
  const clusteringResponse = page.waitForResponse((response) => response.url().endsWith("/clustering") && response.request().method() === "POST")
  await page.getByRole("button", { name: "Submit clustering", exact: true }).click()
  const accepted = await clusteringResponse
  expect(accepted.status()).toBe(202)
  const analysis = await accepted.json()
  expect(analysis).toMatchObject({ tool: "gromacs", operation: "trajectory_clustering", source_job_id: chained })
  await expect(page).toHaveURL(new RegExp(`/jobs/${analysis.job_id}$`))
  await expect(page.getByRole("link", { name: "View source simulation", exact: true })).toHaveAttribute("href", `/tools/gromacs/jobs/${chained}`)
  await completed(analysis.job_id)
  await page.getByRole("button", { name: "Refresh", exact: true }).click()
  ...
  await expect(page.getByRole("img", { name: /production trajectory plot/ })).toHaveCount(0)
  const clusterDownloadEvent = page.waitForEvent("download")
  await page.getByRole("button", { name: "Download result", exact: true }).click()
  const clusterDownload = await clusterDownloadEvent
  expect(clusterDownload.suggestedFilename()).toBe("result.zip")
  const parts: Buffer[] = []
  for await (const part of await clusterDownload.createReadStream()) parts.push(Buffer.from(part))
  const clusterArchive = Buffer.concat(parts)
  expect(clusterArchive.subarray(0, 2).toString()).toBe("PK")
  // The deterministic fixture deliberately stores these members uncompressed.
  expect(clusterArchive.toString()).toContain("clusters.csv")
  expect(clusterArchive.toString()).toContain("frame,time_ns,cluster_id,is_medoid\n0,0.0,1,yes\n1,0.1,1,\n2,0.2,2,yes\n")
  expect(clusterArchive.toString()).toContain("-frame_0.pdb")
  expect(clusterArchive.toString()).toContain("-frame_2.pdb")
  expect(clusterArchive.toString()).toContain("provenance.json")
  const finishedAnalysis = await (await context.request.get(`/api/v1/jobs/${analysis.job_id}`)).json()
